...
<body>
	<form action="dt2/bind.wx" method="post">
    <div>db的连接值<input type="text" name='urs' value='${db?default("")}' placeholder="请输入帐号"></div>
    <div>conf的值<input type='text'  name='passwd' value='${env?default("")}' placeholder="请输入密码"><input type='hidden' value='${env?default("")}' name="st"></div>
